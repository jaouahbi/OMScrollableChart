...
extension CGRect {
    static func clampFrame(_ frame: CGRect, inBounds bounds: CGRect) -> CGRect {
        let center: CGPoint = CGPoint(x: Swift.max(bounds.minX + frame.width*0.5, Swift.min(frame.midX, bounds.maxX - frame.width*0.5)),
                                      y: Swift.max(bounds.minY + frame.height*0.5, Swift.min(frame.midY, bounds.maxY - frame.height*0.5)))
        return CGRect(x: center.x-frame.width*0.5, y: center.y-frame.height*0.5, width: frame.width, height: frame.height)
    }
    
    static func clampFrameToCenter(_ frame: CGRect, inBounds bounds: CGRect) -> CGPoint {
        let center: CGPoint = CGPoint(x: Swift.max(bounds.minX + frame.width*0.5, Swift.min(frame.midX, bounds.maxX - frame.width*0.5)),
                                      y: Swift.max(bounds.minY + frame.height*0.5, Swift.min(frame.midY, bounds.maxY - frame.height*0.5)))
        return center
    }
    
    static func moveFrame(_ frame: CGRect, by translation: CGPoint, constrainedTo bounds: CGRect) -> CGRect {
        var newFrame = frame
        newFrame.origin.x += translation.x
        newFrame.origin.y += translation.y
        return clampFrame(newFrame, inBounds: bounds)
    }
    static func moveCenter(_ frame: CGRect, by translation: CGPoint, constrainedTo bounds: CGRect) -> CGPoint {
        var newFrame = frame
        newFrame.origin.x += translation.x
        newFrame.origin.y += translation.y
        return clampFrameToCenter(newFrame, inBounds: bounds)
    }
}
