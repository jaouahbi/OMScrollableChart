...
extension Point3D {
    ...
    func d(point: Point3D) -> CGFloat {
        self.sub(point: point).norm()
    }

    func add(scalar: CGFloat) -> Point3D {
        var newv = Point3D(x: 0, y: 0, z: 0)
...
func + (left: CGAffineTransform, right: CGPoint) -> CGAffineTransform {
    left.translatedBy(x: right.x, y: right.y)
}
 ...
 */
func * (left: CGAffineTransform, right: CGPoint) -> CGAffineTransform {
    left.scaledBy(x: right.x, y: right.y)
}
...
extension CGPoint {
    ...
    func distance(to point: CGPoint) -> CGFloat {
        return self.differential(to: point).hypotenuse
    }
