...
class CAAnimationHandler: NSObject, CAAnimationDelegate {
    ...
    /// animating: A block (closure) object to be executed when the animation is animating.
    ///  This block has no return value and takes a single CGFloat argument that indicates t
    ///  he progress of the animation (From 0 ..< 1)
...
public extension CAAnimation {
    ...
    /// A block (closure) object to be executed when the animation is animating.
    /// This block has no return value and takes a single CGFloat argument that indicates the progress of the animation (From 0 ..< 1)
    var animating: ((CGFloat) -> Void)? {
        set {
			if let animationDelegate = delegate as? CAAnimationHandler {
